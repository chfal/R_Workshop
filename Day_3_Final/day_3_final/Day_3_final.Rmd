...
ggplot(data = iris, mapping = aes(x=sepal_width, y=petal_width))

#start by setting up ggplot: set the axes and add some color

ggplot(data = iris, mapping = aes(x=sepal_width, y=petal_width, color=species)) +
  geom_jitter()
...
ggplot(data = iris, mapping = aes(x=sepal_width, y=petal_width, color=species)) +
  ...
  geom_smooth(method="lm") +
  facet_grid(~species) +
  labs(x= "sepal length", y="petal width", title="sepal and peta width of iris species")


petal_sepal <- lm(petal_width~sepal_width, data=iris)
...
virginica <- iris_mutated %>% filter(species=="virginica")


ggplot(data=virginica, aes(x=sepal_area, y=petal_area)) +
  geom_jitter(color="skyblue") +
  geom_smooth(method="lm") +
  labs(x="Sepal Area", y="Petal Area", title="Sepal Area vs. Petal Area for Iris virginica")

sepal_petal_area <- lm(petal_area~sepal_area, data=virginica)

lm(petal_area~sepal_area, data=virginica)

summary(sepal_petal_area)

plot(sepal_petal_area)
```
